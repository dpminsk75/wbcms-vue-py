      ...
      <table class="table table-striped table-bordered table-hover kv-grid-table mb-0" style="font-size:12px; margin:0; font-family:&quot;Segoe UI&quot;,Roboto,Helvetica,Arial,sans-serif">
        <thead>
          <tr style="background:#f8f7fb; font-size:12px">
            <th style="padding:6px 8px; text-align:left; width:350px">Товар / Артикул</th>
            <th style="padding:6px 8px; text-align:center; width:90px">Кол-во</th>
            <th style="padding:6px 8px; text-align:right; width:85px">Σ в РЦ</th>
            <th style="padding:6px 8px; text-align:right; width:85px">Σ к опл</th>
            <th style="padding:6px 8px; text-align:right; width:90px">Σ WB нам</th>
            <th style="padding:6px 8px; text-align:right; width:70px">Цена</th>
            <th style="padding:6px 8px; text-align:right; width:60px">СПП, %</th>
            <th style="padding:6px 8px; text-align:right; width:75px">Цена пр.</th>
            <th style="padding:6px 8px; text-align:right; width:75px">К опл</th>
          </tr>
        </thead>
        <tbody>
          <tr class="table-warning kv-page-summary w0" style="font-weight:700; font-size:12px; text-align:right">
            <td style="padding:6px 8px; color:#7c1af8; text-align:left; white-space:nowrap">Итого {{ totalCnt }} пр | Σ в РЦ: {{ fmt1(totalPwd) }} | Σ прод: {{ fmt1(totalFp) }} | WB нам: {{ fmt1(totalForpay) }}</td>
            ...
            <td style="padding:6px 8px; text-align:right">{{ fmt0(totalForpay) }}</td>
          ...
          <tr v-for="r in displayRows" :key="r.nm_id" style="font-size:12px">
            ...
            <td style="padding:5px 8px; text-align:right">{{ fmt0(r.forpay) }}</td>
            ...
            <td style="padding:5px 8px; text-align:right">{{ Math.round(r.afp||0).toLocaleString('ru-RU') }}</td>
            <td style="padding:5px 8px; text-align:right; font-weight:700">{{ Math.round(r.aforpay||0).toLocaleString('ru-RU') }}</td>
          </tr>
        </tbody>
      </table>
    </div>
