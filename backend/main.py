...
from backend.routers import auth_router, companies_router, dashboard_router, admin_router, tags_router, wb_search_router, cost_router, wb_orders_router, wb_sales_router, feedback_router, reply_rules_router, wb_tokens_router, wb_tokens_expiring_router, competitor_router, ai_jobs_router, seo_router, seo_models_router, cms_router, ext_router, ext_tokens_admin_router, ext_diag_admin_router, ext_download_router, ext_filters_router
...
async def _recover_ai_jobs():
    """Вариант B: зависшие queued/running джобы (рестарт между делом) → interrupted."""
    try:
        from backend.services import ai_job_service as _jobs
        await _jobs.recover_interrupted()
    except Exception:
        pass
