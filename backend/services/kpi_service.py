...
class KpiService:
    ...
    async def get_30d_chart(self):
        sql = text(f"""
            ...
            WHERE sdate BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE() - INTERVAL 1 DAY{self._where()}
            GROUP BY sdate ORDER BY sdate
        """)
        rows = (await self.db.execute(sql, self._params())).mappings().all()
    ...
    async def get_30d_kpi(self):
        sql = text(f"""
            ...
            WHERE sdate BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE() - INTERVAL 1 DAY{self._where()}
        """)
        row = (await self.db.execute(sql, self._params())).mappings().first()
        # count orders — как SiteController.php:232
        cnt = (await self.db.execute(text(f"SELECT COUNT(*) as cnt FROM wb_order WHERE date BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE() - INTERVAL 1 DAY{self._where()}"), self._params())).scalar()
