...
class WbDetailService:
    ...
    async def get_warehouse_stocks(self, nm_id: int):
        ...
        last_date_sql = text("SELECT date FROM wb_stocks ORDER BY date DESC LIMIT 1")
        last_date = (await self.db.execute(last_date_sql)).scalar()
        if not last_date:
            return []
        sql = text("""SELECT warehouse_name, quantity FROM wb_stocks
                      WHERE nm_id=:nm_id AND date=:d AND quantity>0 ORDER BY quantity DESC""")
        rows = (await self.db.execute(sql, {"nm_id": nm_id, "d": last_date})).mappings().all()
    ...
    async def get_in_way_stocks(self, nm_id: int):
        ...
        if not last_date:
            return []
        sql = text("""SELECT warehouse_name, in_way_to_client, in_way_from_client FROM wb_stocks
                      WHERE nm_id=:nm_id AND date=:d AND (in_way_to_client>0 OR in_way_from_client>0)
    ...
    async def get_adv(self, nm_id: int, date_from: str, date_to: str):
        ...
        sql = text("""
            ...
            INNER JOIN wb_campaign c ON c.campaign_id=s.campaign_id
            LEFT JOIN wb_campaign_item i ON i.campaign_id=s.campaign_id AND n.nm_id=i.nm_id
        ...
        """)
        rows = (await self.db.execute(sql, {"nm_id": nm_id, "d1": f"{date_from} 00:00:00", "d2": f"{date_to} 23:59:59"})).mappings().all()
        return [dict(r) for r in rows]
    ...
    async def get_order_stats(self, nm_id: int, date_from: str, date_to: str):
        ...
        sql = text("""
            ...
            WHERE o.nm_id=:nm_id AND o.date BETWEEN :d1 AND :d2
            GROUP BY o.nm_id
        """)
        row = (await self.db.execute(sql, {"nm_id": nm_id, "d1": f"{date_from} 00:00:00", "d2": f"{d_to.isoformat()} 23:59:59"})).mappings().first()
        if not row:
    ...
    async def get_funnel(self, nm_id: int, date_from: str, date_to: str):
        ...
        sql = text("""
            ...
            WHERE o.nm_id=:nm_id AND o.date BETWEEN :d1 AND :d2
        """)
        r = (await self.db.execute(sql, {"nm_id": nm_id, "d1": f"{date_from} 00:00:00", "d2": f"{date_to} 23:59:59"})).mappings().first()
        d = dict(r) if r else {}
    ...
    async def get_daily_orders(self, nm_id: int, date_from: str, date_to: str):
        sql = text("""SELECT DATE(date) as odate, nm_id,
                      ...
                      FROM wb_order WHERE nm_id=:nm_id AND date BETWEEN :d1 AND :d2 GROUP BY DATE(date), nm_id ORDER BY odate DESC""")
        rows = (await self.db.execute(sql, {"nm_id": nm_id, "d1": f"{date_from} 00:00:00", "d2": f"{date_to} 23:59:59"})).mappings().all()
        return [dict(r) for r in rows]

    async def get_daily_sales(self, nm_id: int, date_from: str, date_to: str):
        sql = text("""SELECT DATE(date) as odate, nmId as nm_id,
                      ...
                      FROM wb_sales WHERE nmId=:nm_id AND date BETWEEN :d1 AND :d2 GROUP BY DATE(date), nmId ORDER BY odate DESC""")
        rows = (await self.db.execute(sql, {"nm_id": nm_id, "d1": f"{date_from} 00:00:00", "d2": f"{date_to} 23:59:59"})).mappings().all()
        return [dict(r) for r in rows]
    ...
    async def get_weekly_finance(self, nm_id: int, date_from: str, date_to: str):
        ...
        sql = text("""SELECT DATE_SUB(sdate, INTERVAL WEEKDAY(sdate) DAY) as sdate,
                      ...
                      FROM agg_daily_summary WHERE nm_id=:nm_id AND sdate BETWEEN :d1 AND :d2
